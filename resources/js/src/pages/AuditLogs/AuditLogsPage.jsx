  ...
  return (
    <div className="p-4 sm:p-6 space-y-5">
      <div>
        ...
        <p className="text-sm text-gray-500 dark:text-slate-400 mt-0.5">System activity and change history</p>
      </div>
          ...
          <Tbody>
            {isLoading ? (
              ...
              <Tr key={log.id} onClick={() => setDetail(log)}>
                ...
                <Td>
                  <span className="text-xs bg-gray-100 dark:bg-slate-700 text-gray-600 dark:text-slate-300 px-2 py-0.5 rounded capitalize">
                    {log.module ?? log.subject_type?.split('\\').pop()}
                  </span>
                </Td>
                <Td>
                  <p className="text-sm text-gray-600 dark:text-slate-300 truncate max-w-xs">
                ...
                <Td>
                  <p className="text-sm dark:text-slate-300">{formatDate(log.created_at)}</p>
                  <p className="text-xs text-gray-400 dark:text-slate-500">
                    {log.created_at
        ...
        {detail && (
          <div className="p-5 space-y-4">
            <div className="grid grid-cols-2 gap-4 text-sm">
              <div>
                <p className="text-xs text-gray-400 dark:text-slate-500 uppercase font-semibold mb-0.5">User</p>
                <p className="font-medium text-gray-900 dark:text-white">
                  {detail.user?.name ?? detail.causer?.name ?? 'System'}
                </p>
              </div>
              <div>
                <p className="text-xs text-gray-400 dark:text-slate-500 uppercase font-semibold mb-0.5">Action</p>
                <Badge variant={ACTION_BADGE[detail.action] ?? 'default'}>{detail.action ?? detail.event}</Badge>
              </div>
              <div>
                <p className="text-xs text-gray-400 dark:text-slate-500 uppercase font-semibold mb-0.5">Module</p>
                <p className="text-gray-900 dark:text-white">
                  {detail.module ?? detail.subject_type?.split('\\').pop()}
                </p>
              </div>
              <div>
                <p className="text-xs text-gray-400 dark:text-slate-500 uppercase font-semibold mb-0.5">Record ID</p>
                <p className="font-mono text-xs text-gray-700 dark:text-slate-300">
                  {detail.record_id ?? detail.subject_id}
                </p>
              </div>
              <div className="col-span-2">
                <p className="text-xs text-gray-400 dark:text-slate-500 uppercase font-semibold mb-0.5">Description</p>
                <p className="text-gray-700 dark:text-slate-300">{detail.description ?? detail.log_name}</p>
              </div>
            </div>
            {(detail.old_values ?? detail.properties?.old) && (
              <div>
                <p className="text-xs text-gray-400 dark:text-slate-500 uppercase font-semibold mb-2">Changes</p>
                <pre className="bg-gray-50 dark:bg-slate-900 rounded-lg p-3 text-xs overflow-auto max-h-48 text-gray-700 dark:text-slate-300 border border-gray-100 dark:border-slate-700">
