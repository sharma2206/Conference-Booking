...
export default function UsersPage() {
  ...
  const deleteMutation = useMutation({
    ...
    onSuccess: () => { toast.success('User deleted'); qc.invalidateQueries({ queryKey: ['users'] }); setDeleteTarget(null); },
  ...
  return (
    <div className="p-6 space-y-5">
                ...
                <Td className="text-right">
                  <div className="flex items-center justify-end gap-1">
                    ...
                    {hasPermission('user.delete') && (
                      <button onClick={() => setDeleteTarget(u)} className="p-1.5 rounded-lg text-gray-400 hover:text-red-600 hover:bg-red-50 transition-colors">
      ...
      {/* Form Modal */}
      <Modal isOpen={formModal} onClose={() => setFormModal(false)} title={editUser ? 'Edit User' : 'Add User'} size="lg">
        <form onSubmit={handleSubmit(onSubmit)} className="p-5 grid grid-cols-2 gap-4">
          <Input label="Full Name" required error={errors.name?.message} {...register('name')} className="col-span-2" />
          <Input label="Email" required type="email" error={errors.email?.message} {...register('email')} />
          <Input label="Password" type="password" placeholder={editUser ? 'Leave blank to keep' : 'Min 8 chars'} error={errors.password?.message} {...register('password')} />
          ...
          <div className="col-span-2 flex justify-end gap-2 pt-2 border-t border-gray-100">
            ...
            <Button type="submit" loading={saveMutation.isPending}>{editUser ? 'Update User' : 'Create User'}</Button>
          </div>
        </form>
      </Modal>
